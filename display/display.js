...
function propertySlide(p){
  ...
  return `<section class="screen property-slide">
    <div class="photo-stage">
      ...
      <div class="photo-main">${imgs.map((img,i)=>`<img class="property-photo ${i===0?'active-photo':''}" src="${imageSrc(img)}" alt="${esc(p.city)} – ${esc(p.type)}" onerror="this.src='../assets/images/logo-brand.jpeg'">`).join('')}</div>
    ...
    <div class="panel">
      ...
      <div class="contact"><strong>☎ +36 70 319 6582</strong>7900 Szigetvár, József A. utca 35.</div>
    </div>
  </section>`;
}
function adSlide(ad){
...
function infoSlide(kind, eyebrow, title, body, extra=''){
  return `<section class="screen info-slide info-${kind}">
    ...
    <div class="info-card">
      ...
      <div class="info-brand">SZIGET-BARACSI INGATLAN <span>•</span> Otthon. Érték. Bizalom.</div>
    </div>
  </section>`;
}
...
function restartProgress(){const bar=root.querySelector('.progress');if(!bar)return;bar.style.animation='none';void bar.offsetWidth;bar.style.animation=`progress ${SLIDE_DURATION}ms linear forwards`;}
async function init(){
  await loadLiveData();
  const properties=await loadJson('../data/properties.json',[]);
  const ads=(await loadJson('ads.json',[])).filter(x=>x.active!==false);
  const active=properties.filter(p=>p.status==='active');
  const info=buildInfoItems(); const sequence=[]; let adIndex=0,infoIndex=0;
  active.forEach((p,index)=>{
    sequence.push({type:'property',html:propertySlide(p)});
    if((index+1)%3===0&&ads.length){sequence.push({type:'ad',html:adSlide(ads[adIndex%ads.length])});adIndex++;}
    // every 6 properties insert one live information card
    if((index+1)%6===0&&info.length){sequence.push(info[infoIndex%info.length]);infoIndex++;}
  });
  if(active.length%3!==0&&ads.length)sequence.push({type:'ad',html:adSlide(ads[adIndex%ads.length])});
  // ensure every live card appears at least once in a full rotation
  while(infoIndex<info.length){sequence.push(info[infoIndex++]);}
  root.innerHTML=sequence.map(x=>x.html).join('')+overlayHtml()+'<div class="progress"></div>';
  updateOverlay();setInterval(updateOverlay,1000);
  const slides=[...root.querySelectorAll('.screen')];if(!slides.length)return;
  let current=0;slides[0].classList.add('active');startPhotoRotation(slides[0]);restartProgress();
  function show(nextIndex){
    const oldSlide=slides[current],newSlide=slides[nextIndex];if(oldSlide===newSlide)return;
    const effect=slideEffects[slideEffectIndex++%slideEffects.length];newSlide.classList.add('active');newSlide.style.zIndex='4';oldSlide.style.zIndex='3';
    if(reduceMotion){oldSlide.classList.remove('active');}else{
      oldSlide.animate(transitionFrames(effect,false),{duration:650,easing:'cubic-bezier(.4,0,.2,1)',fill:'forwards'});
      newSlide.animate(transitionFrames(effect,true),{duration:1100,easing:'cubic-bezier(.16,1,.3,1)',fill:'both'}).finished.finally(()=>{oldSlide.classList.remove('active');oldSlide.getAnimations().forEach(a=>a.cancel());newSlide.getAnimations().forEach(a=>a.cancel());oldSlide.style.zIndex='';newSlide.style.zIndex='';});
    }
    current=nextIndex;startPhotoRotation(newSlide);restartProgress();
  }
  setInterval(()=>show((current+1)%slides.length),SLIDE_DURATION);
}
init();
