...
const slideEffects = ['fade','slide-left','slide-right','slide-up','zoom','wipe'];
  ...
  function show(nextIndex){
    ...
    if(oldSlide!==newSlide){
      ...
      if(reduceMotion){
        ...
        const incoming=transitionFrames(effect,true);
        const outgoing=transitionFrames(effect,false);

        const inAnim=newSlide.animate(incoming,{
          duration:1050,
          easing:'cubic-bezier(.22,.7,.25,1)',
          fill:'both'
        });

        oldSlide.animate(outgoing,{
          duration:1050,
          easing:'cubic-bezier(.22,.7,.25,1)',
          fill:'both'
        });

        inAnim.finished.finally(()=>{
          oldSlide.classList.remove('active');
          oldSlide.style.zIndex='';
          newSlide.style.zIndex='';
          oldSlide.getAnimations().forEach(a=>a.cancel());
