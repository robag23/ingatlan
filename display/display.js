...
const slideEffects = ['fade','slide-left','slide-right','slide-up','zoom'];
const photoEffects = ['fade','slide-left','slide-right','zoom','soft-blur'];
...
function setupFullscreenToggle(){
  ...
  if(!btn){
    ...
    btn.setAttribute('title','Teljes képernyő');
  ...
  function refreshFullscreenButton(){
    ...
    btn.style.display=active ? 'none' : 'grid';
  }
...
function overlayHtml(){return `
  <img class="corner-logo" src="../assets/images/logo-brand.jpeg" alt="Sziget-Baracsi Ingatlan">
  <div class="live-corner">
    <span class="live-clock"></span>
    <span class="live-date"></span>
    <span class="live-temp"></span>
  </div>`;}
...
function startPhotoRotation(slide){
  ...
  photoTimer=setInterval(()=>{
    ...
    if(reduceMotion){
      ...
      const frames=photoFrames(effect,true);
      newPhoto.animate(frames,{duration:900,easing:'cubic-bezier(.22,.7,.25,1)',fill:'both'}).finished.finally(()=>{
        oldPhoto.classList.remove('active-photo');oldPhoto.style.zIndex='';newPhoto.style.zIndex='';
...
async function init(){
  ...
  // A külső adatok hibája/kimaradása nem állíthatja meg a kijelzőt.
  await loadLiveData();
  ...
  active.forEach((p,index)=>{
    const html=propertySlide(p).replace(
      '<section class="screen property-slide ',
      '<section data-duration="'+PROPERTY_DURATION+'" class="screen property-slide '
    );
  ...
  // Még teljes adatkimaradáskor se legyen üres képernyő.
  if(!sequence.length){
    sequence.push({
      type:'promo',
      html:promoSlide(
        'sell',
        'SZIGET-BARACSI INGATLAN',
        'Otthon. Érték. Bizalom.',
        'Ingatlanok Szigetváron és környékén.',
        ['☎ +36 70 319 6582','🌐 szigetvarhomes.com']
      )
    });
  }
  ...
  // Nem várunk korlátlanul külső képekre.
  await preloadDisplayImages();

  updateOverlay();
  setInterval(updateOverlay,1000);
  setInterval(applySeasonalTheme,60000);
  ...
  function durationFor(slide){
    const value=Number(slide?.dataset?.duration || PROPERTY_DURATION);
    return Number.isFinite(value) && value>=5000 ? value : PROPERTY_DURATION;
  }

  function clearSlideAnimation(slide){
    if(!slide) return;
  ...
  function activateSlide(index,animate=true){
    ...
    // Kritikus stabilitási szabály:
    // először az új dia lesz látható, csak utána kapcsoljuk ki a régit.
    const next=slides[index];
    clearSlideAnimation(next);
    next.classList.add('active');
    next.style.zIndex='5';

    // Force layout so every animation always starts from a clean state.
    void next.offsetWidth;

    slides.forEach((slide,i)=>{
      if(i!==index){
        slide.classList.remove('active');
        clearSlideAnimation(slide);
      }
    });
    ...
    if(animate && !reduceMotion){
      ...
      next.classList.add('motion-'+effect);

      // Never use animation.finished promises here:
      // long-running browser/display sessions can delay them and leave
      // stale callbacks that later hide the currently active slide.
      transitionTimer=setTimeout(()=>{
        if(slides[current]===next){
          next.classList.remove('motion-'+effect);
  ...
  // Hosszú futású watchdog:
  // ha böngésző/GPU/animáció miatt bármikor eltűnne az aktív dia,
  // 2 másodpercen belül helyreállítja.
  function ensureVisibleSlide(){
    const activeSlide=slides[current] || slides[0];
    const hasActive=slides.some(s=>s.classList.contains('active'));

    if(!hasActive || !activeSlide.classList.contains('active')){
      slides.forEach(s=>{
        s.classList.remove('active');
        clearSlideAnimation(s);
      });
      activeSlide.classList.add('active');
      activeSlide.style.zIndex='5';
    }

    // Hibás inline opacity/visibility se hagyhassa láthatatlanná a diát.
    const cs=getComputedStyle(activeSlide);
    if(cs.display==='none' || cs.visibility==='hidden' || Number(cs.opacity)<0.05){
      clearSlideAnimation(activeSlide);
      activeSlide.classList.add('active');
      activeSlide.style.zIndex='5';
    }
  }
  ...
  scheduleNext();
  watchdogTimer=setInterval(ensureVisibleSlide,2000);

  // Tab/fullscreen visszatéréskor azonnali helyreállítás.
  ...
  document.addEventListener('fullscreenchange',()=>{
    setTimeout(ensureVisibleSlide,100);
  });
}
