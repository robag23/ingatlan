...
function propertySlide(p){
  ...
  const description=p.description||'További részletekért, megtekintésért és az ingatlannal kapcsolatos kérdésekért keressen minket bizalommal. Személyesen segítünk az ingatlan megismerésében és a következő lépésekben.';
  return `<section class="screen property-slide ${hasPhotos?'has-photo':'missing-photo'}">
    <div class="photo-stage">
      ...
      <div class="photo-main">${imgs.map((img,i)=>`<img class="property-photo ${i===0?'active-photo':''}" src="${imageSrc(img)}" alt="${esc(p.city)} – ${esc(p.type)}" onerror="this.onerror=null;this.src='../assets/images/logo-brand-new.png';this.closest('.property-slide')?.classList.add('missing-photo')">`).join('')}</div>
...
function buildInfoItems(){
  ...
  items.push({type:'info',html:infoSlide('nameday','MAI NÉVNAP',todaysNames.length?todaysNames.map(esc).join(' • '):'Szép napot kívánunk!',`<p class="lead">Sok szeretettel köszöntjük a mai névnaposokat!</p><p>Kívánunk örömökben gazdag, szép napot, jó egészséget, szeretetet és sok emlékezetes pillanatot.</p>`,'Boldog névnapot kíván a Sziget-Baracsi Ingatlan!')});
...
function themeForDate(now){
  ...
  if(m===9 && d>=4 && d<=13) return 'zrinyi';
  if((m===9 && d>=15) || (m===10 && d<=31)) return 'harvest';
...
function setupFullscreenToggle(){
  ...
  const sync=()=>{
    ...
    if(label) label.textContent=on?'KILÉPÉS':'TELJES KÉPERNYŐ';
  ...
  btn.addEventListener('click',async()=>{
    try{
      ...
      console.warn('Fullscreen nem indítható ebben a böngészőben:',e);
    }
    sync();
  });
