...
async function loadProperties(status){
  ...
  try{
    ...
    // Főoldal: csak kiemelt, legalább 50 millió Ft-os aktív ingatlanok.
    if(el.dataset.homeFeatured === 'true'){
      list = list
        .filter(x => x.featured === true && numericPrice(x.price) >= 50000000)
        .sort((a,b) => numericPrice(b.price) - numericPrice(a.price));
    }
